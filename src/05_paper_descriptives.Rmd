---
title: "05_paper_descriptives.Rmd"
...
Goal of the Script: Produce descriptive statistics for the paper
...
```{r}
# Gold standard
gs <- read.csv("../data/processed/gold_standard/gold_standard.csv")

# Annotation dataset
ads <- read.csv("../data/processed/gold_standard/annotation_dataset.csv")
```
...
First compute agreement between experts and then count their agreement with non-experts
```{r}
agreement <- ads %>% ungroup() %>%
  # excluding reports without highlighted rows
  filter(non_exp_filter_rec == "Yes") %>% 
  
  # compute agreement between experts
  mutate(exp_agreement = case_when(
    is.na(exp_group_1_value) & is.na(exp_group_2_value) ~ TRUE,
    !is.na(exp_group_1_value) & is.na(exp_group_2_value) ~ FALSE,
    is.na(exp_group_1_value) & !is.na(exp_group_2_value) ~ FALSE,
    exp_group_1_value == exp_group_2_value ~ TRUE,
    exp_group_1_value != exp_group_2_value ~ FALSE
  ),
  
  # compute agreement between experts and non-experts
  exp_vs_non_exp_agreement = case_when(
    exp_agreement & (exp_group_1_value_who_is_right == "Ann1" | exp_group_1_value_who_is_right == "Ann2") ~ TRUE,
    exp_agreement & exp_group_1_value_who_is_right == "Neither" ~ FALSE,
    .default = FALSE
  ))
```

```{r}
agreement %>% filter(exp_agreement) %>% group_by(exp_vs_non_exp_agreement) %>% summarize(n = n(), .groups = "drop") %>% reframe(percentage = n / sum(n) * 100)
```
Alternative:
