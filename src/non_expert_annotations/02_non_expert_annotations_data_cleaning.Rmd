...
A non-x row is a page-year-scope combination. Correct ID because currently string "110.0", so split off ".0"
For the x-rows, transform the ID to a similar variable by concatenating page, year and scope.
...
```{r}
similar_rows_by_reporting_type <- left_join(similar_rows_by_reporting_type, df5, by = c("ID", "report_name", "llm_scope", "llm_year", "llm_value", "llm_unit", "page_used", "metric_name", "reporting_type"))

similar_rows_by_metric_name <- left_join(similar_rows_by_metric_name, df5, by = c("ID", "report_name", "llm_scope", "llm_year", "llm_value", "llm_unit", "page_used", "metric_name", "reporting_type"))
```

Remove similar_rows_by_reporting_type and similar_rows_by_metric_name from df5 to add corrected rows later again

```{r}
df6 <- df5 %>% anti_join(similar_rows_by_reporting_type, by = c("ID", "report_name", "llm_scope", "llm_year", "llm_value", "llm_unit", "page_used", "metric_name", "reporting_type")) %>%
...
```{r}
df7 <- df6 %>% bind_rows(similar_rows_by_reporting_type)
```
...
```{r}
df8 <- df7 %>% bind_rows(similar_rows_by_metric_name)
```
...
pivot_combined <- pivot_w_split(df8, pivot_vars)
```
...
```{r}
extra_columns_corrections <- df8 %>% right_join(pivot_corrected, by = c("report_name", "ID", "page_used", "llm_year", "llm_scope", "llm_value", "llm_unit", "annotator_ID")) %>% select(-ends_with(".x"), -ends_with(".y"))
...
df9 <- df8 %>% anti_join(pivot_corrected, by = c("report_name", "ID", "page_used", "llm_year", "llm_scope", "llm_value", "llm_unit", "annotator_ID")) %>% bind_rows(pivot_corrected)
```
...
```{r}
df9 %>% filter(is.na(value_correct) & !is.na(corrected_value))

```
Set value_correct to "No"
```{r}
df10 <- df9 %>% mutate(value_correct = ifelse(is.na(value_correct) & !is.na(corrected_value), "No", value_correct))
```
- Is value correct == "Yes" and corrected_value not NA?
```{r}
df10 %>% filter(value_correct == "Yes" & !is.na(corrected_value))
```

For unit:
- Is value_correct NA for corrected_value not NA?
```{r}
df10 %>% filter(is.na(unit_correct) & !is.na(corrected_unit))
```
```{r}
df11 <- df10 %>% mutate(unit_correct = ifelse(is.na(unit_correct) & !is.na(corrected_unit), "No", unit_correct))
```

- Is unit_correct set to "Yes" and corrected_unit not NA?
```{r}
df11 %>% filter(unit_correct == "Yes" & !is.na(corrected_unit))
```

For page:
- Is page_correct NA for corrected_page not NA?
```{r}
df11 %>% filter(is.na(page_correct) & !is.na(corrected_page))
```

- Is page_correct set to "Yes" and corrected_page not NA?
```{r}
df11 %>% filter(page_correct == "Yes" & !is.na(corrected_page))
```
...
# Save cleaned data
```{r}
write.csv(df12,"../data/processed/non_expert_annotations/non_expert_annotations_cleaned.csv", row.names = FALSE)
```
