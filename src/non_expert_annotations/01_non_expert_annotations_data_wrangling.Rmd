...
# Goal of script: Read every annotation file in a folder, extract metadata, and append all data into a single Excel file
...
read_process_file <- function(file_path) {
  ...
  data_df <- data_df %>% filter(!is.na(.[[1]]))
...
process_excel_files <- function(folder_path, output_file_name) {
  ...
  # Map the function over all file paths, read and process each file
  all_dataframes <- map_df(file_paths, read_process_file)
  
  # Split off unnecessary columns
  ...
  write.csv(all_dataframes, output_file_path)
