---
title: "gs5_expert_disagreement"
author: "Anna Steinberg"
date: "`r Sys.Date()`"
output: distill::distill_article
---

## Goal of the script: Determine expert disagreement by identifying reports where experts disagree on the true value
...
read_and_compare <- function(folder_path) {
  #Read in spreadsheet which links the two expert annotation templates of each group to the report name
  template_naming <- read_excel("../../data/templates/filled_templates_main.xlsx", sheet = 1)
  
  #TODO: Change folder path
  process_order <- template_naming %>% arrange(report_name) %>% mutate(path = paste0(folder_path, "/", folder_name, "/", new_report_name, ".xlsx"))
...
```{r}
# TODO: Change to path to true data
...
```{r}
exp %>% group_by(`Report Name`) %>% filter(any(agreement_value_unit == FALSE) | any(at_least_one_group_flagged_unclear == TRUE)) %>% distinct(`Report Name`)
```
...
```{r}
exp %>% group_by(`Report Name`) %>% filter(any(agreement_value_unit_page == FALSE) | any(at_least_one_group_flagged_unclear == TRUE)) %>% distinct(`Report Name`) %>% arrange(`Report Name`)
```
Missing:
...
- Create row highlighting based on expert group disagreeement or disagreement on unclear record
- Rearrange dataframe
...
```{r}
write.csv(exp, "../../data/processed/expert_annotations.csv")
```
...
Save dataframe
```{r}
write_csv(exp_discussion, "../../data/templates/expert_disagreement.csv")
```