...
## Inspect page columns
```{r}
# Inspect page entries for each page-related column

annotations %>% distinct(page_used)
annotations %>% distinct(corrected_page_1)
annotations %>% distinct(corrected_page_2)
annotations %>% distinct(Page.corrected..fill.in.if.necessary..g1)
annotations %>% distinct(Page.corrected..fill.in.if.necessary..g2)
annotations %>% distinct(True.page)
```
Remove ".0" from page values for columns Page.corrected..fill.in.if.necessary..g1 and True.page
```{r}
annotations <- annotations %>% mutate(
  Page.corrected..fill.in.if.necessary..g1 = str_replace(Page.corrected..fill.in.if.necessary..g1, "\\.0", ""),
  True.page = str_replace(True.page, "\\.0", "")
)
```
