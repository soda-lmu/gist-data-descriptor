...
The resulting full annotations dataframe is used to create gold standard and annotations dataset. The combine the datasets back together a merge id is created. 
...
### Solve mismatch
For some reason, in the expert annotations the columns of the non-experts are sometimes switched compared to the non-expert annotations. We need to resolve this by 
...
for (var in concerned_variables) {
  test <- swap_values(annotations, mismatch, var, "value", "Value.Ann")
}
```
...
More checks
```{r}
test %>% filter(value_correct_1 != Value.correct..Yes.No..Ann.1) %>% select(report_name, value_correct_1, corrected_value_1, Value.correct..Yes.No..Ann.2, Value.corrected.Ann.2, Value.correct..Yes.No..Ann.1, Value.corrected.Ann.1, value_correct_2, corrected_value_2)
```
```{r}
test %>% filter(value_correct_2 != Value.correct..Yes.No..Ann.2)
```
