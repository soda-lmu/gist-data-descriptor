...
Correct discrepancy between report_name and actual company_name for "vital energy" to "uniper"
```{r}
gs_companyname_reportyear <- gs_pagetext %>% 
  mutate(report_name = if_else(
    str_detect(report_name, "vital energy inc_2019_report.pdf"),
    str_replace(report_name, "vital energy inc", "uniper"),
    report_name
    ))

```

Split report_name into company_name and report_year
```{r}
