...
- if disagreement => true_value = NA
  Compute "Value.G1" and "Value.G2" (expert groups 1 and 2)
...
We first create the values according to each non-expert separately, 
i.e. checking their decision in value_correct, 
we select either the value of the LLM or the value the non-expert entered (corrected_value)

Then we inspect the expert review and set the value of each expert group according to their decision in Value..Who.is.right..g1/2
i.e. choose respective value of non-expert or corrected value of expert group
```{r}
...
Now we compute the agreement of non-experts and expert groups according to the computed values
and then use the agreement to decide for each case which value to set as final value
```{r}
annotations <- annotations %>% mutate(
...
## Compute FINAL UNITS
We first create the units according to each non-expert separately, 
i.e. checking their decision in unit_correct, 
we select either the unit of the LLM or the unit the non-expert entered (corrected_unit)

Then we inspect the expert review and set the unit of each expert group according to their decision in Unit..Who.is.right..g1/2
i.e. choose respective unit of non-expert or corrected unit of expert group
```{r}
annotations <- annotations %>% mutate(
...
Now we compute the agreement of non-experts and expert groups according to the computed units
```{r}
annotations <- annotations %>% mutate(
...
Then use the agreement to decide for each case which unit to set as final unit
```{r}
annotations <- annotations %>% mutate(
  true_unit = case_when(
    # non-experts agree and no request for expert review
...
```
Expert Group 2 accidentally entered corrected unit under reasoning column
Check true_unit
```{r}
...
## Compute FINAL PAGE
We first create the page according to each non-expert separately, 
i.e. checking their decision in page_correct, 
we select either the page of the LLM or the page the non-expert entered (corrected_page)

Then we inspect the expert review and set the page of each expert group according to their decision in Value..Who.is.right..g1/2
i.e. choose respective page of non-expert or corrected page of expert group
```{r}
annotations <- annotations %>% mutate(
...
Now we compute the agreement of non-experts and expert groups according to the computed pages
```{r}
annotations <- annotations %>% mutate(
...
Then use the agreement to decide for each case which page to set as final page
```{r}
annotations <- annotations %>% mutate(
  true_page = case_when(
    # non-experts agree and no request for expert review
...
For some reports, no row was flagged for expert review, but the expert adjudication on document-level was requested.
For these reports, the experts did not enter any values, but left document-level comments. 

We inspect these reports manually and decide which values to use according to the experts document-level comments
```{r}
...
```
- "americold" ==> experts confirmed that the values annotated by non-experts are correct
...
```
- "autoneum" ==> experts confirmed that the values annotated by non-experts are correct
...
We keep all non-experts' values for now and add a note for the data analysis. 
...
We keep non-experts' values (NA) for now. 
